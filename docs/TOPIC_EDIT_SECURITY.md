...
- 로그인 없이 신규 제출: 명부에 등록된 회원도 공유 링크로 제출할 수 있으며,
  서버가 32바이트 난수 수정 코드를 한 번 발급한다. 명부 등록 자체로 로그인을 강제하지 않는다.
- 입력한 이름·학번만으로 계정 소유권을 부여하지 않는다. 비로그인 제출은
  `member_id = NULL`인 수정 코드 방식으로 저장한다.
- 비로그인 제출 후 로그인해도 기존 글을 중복 생성하지 않고 수정 코드를 확인한다.
  코드로 작성한 글은 로그인 후 수정해도 코드 방식이 유지된다.
